...
const Product5 = ({ onClickCartAdd, productAdd }) => {
  ...
  const name = productAdd.map((product) => product.name)[4];
  const image = productAdd.map((product) => product.image)[4];
  const price = productAdd.map((product) => product.price)[4];
          ...
          <div className="ProductPage-color">
            ...
            <div>
              ...
              >
                ...
                <option value="다크 블루">다크 블루</option>
              </select>
            </div>
          </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(Product5);
