...
const Product3 = ({ onClickCartAdd, productAdd }) => {
  ...
  const name = productAdd.map((product) => product.name)[2];
  const image = productAdd.map((product) => product.image)[2];
  const price = productAdd.map((product) => product.price)[2];
  return (
    <div>
...
export default connect(mapStateToProps, mapDispatchToProps)(Product3);
