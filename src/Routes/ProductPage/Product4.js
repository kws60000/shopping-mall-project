...
const Product4 = ({ onClickCartAdd, productAdd }) => {
  ...
  const name = productAdd.map((product) => product.name)[3];
  const image = productAdd.map((product) => product.image)[3];
  const price = productAdd.map((product) => product.price)[3];
...
export default connect(mapStateToProps, mapDispatchToProps)(Product4);
