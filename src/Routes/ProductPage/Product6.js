...
const Product6 = ({ onClickCartAdd, productAdd }) => {
  ...
  const name = productAdd.map((product) => product.name)[5];
  const image = productAdd.map((product) => product.image)[5];
  const price = productAdd.map((product) => product.price)[5];
          ...
          <div className="ProductPage-color">
            ...
            <div>
              ...
              >
                ...
                <option value="화이트 & 브라운">화이트 & 브라운</option>
                <option value="블랙 & 그린">블랙 & 그린</option>
              </select>
            </div>
          </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(Product6);
